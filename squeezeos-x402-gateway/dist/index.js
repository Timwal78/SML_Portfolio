...
async function proveCredit(wallet) {
...
async function collectPayment(input) {
    ...
    const external_ref = payAttempt.body?.receipt_id ||
        payAttempt.body?.tx_hash ||
    ...
    const challenge = !payAttempt.ok && !input.x_payment
        ? {
    ...
    return {
        ...
        message: 'Payment path executed for the AGENT business entity (not a human merchant account). Compliance event signed.',
        agent_wallet: wallet.agent_wallet,
        ledger_entry: entry,
...
function generateComplianceLog(input) {
    const wallet = loadOrCreateWallet();
    ...
    return {
        ...payload,
        export_hmac,
        procurement_language: 'Signed agent spend audit trail suitable for pilot procurement packages, SDVOSB capability attachments, and enterprise autonomous-spend control reviews.',
    };
}
async function main() {
    ensureData();
    ...
    server.tool('collect_payment', 'Agent business collects/sends payment before work. Agent wallet — no human merchant account. Records HMAC compliance event.', {
    ...
    server.tool('prove_creditworthiness', 'Returns agent credit score + attestation graph answer to: will this bot pay?', {
        ...
        appendLedger({
            ...
            status: 'checked',
            external_ref: String(credit.score ?? 'n/a'),
        });
        return textResult({
            merchant_question: 'will_this_bot_pay',
            agent_wallet: w,
    ...
    server.tool('generate_compliance_log', 'Export HMAC-signed audit trail of agent transactions for government/enterprise procurement.', {
    ...
    server.tool('agent_business_status', 'Free status: agent wallet id + ledger counts + positioning one-liner.', {}, async () => {
