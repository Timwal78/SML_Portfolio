...
function ensure(): void {
  ...
  if (!existsSync(QUOTA)) writeFileSync(QUOTA, '{}');
}
...
function freeRemaining(tool: string): { ok: boolean; used: number; limit: number } {
  ensure();
  const q = JSON.parse(readFileSync(QUOTA, 'utf8') || '{}') as Record<string, Record<string, number>>;
  const d = dayKey();
  if (!q[d]) q[d] = {};
...
function consumeFree(tool: string): void {
  ensure();
  const q = JSON.parse(readFileSync(QUOTA, 'utf8') || '{}') as Record<string, Record<string, number>>;
  const d = dayKey();
  if (!q[d]) q[d] = {};
  ...
  // drop old days
  for (const k of Object.keys(q)) if (k < d) delete q[k];
  writeFileSync(QUOTA, JSON.stringify(q, null, 2));
}

function sign(body: Omit<LedgerEntry, 'hmac'>): string {
...
async function usaPost(path: string, body: unknown): Promise<any> {
  ...
  if (!res.ok) return { error: `USAspending HTTP ${res.status}`, body: j };
  return j;
}

async function usaGet(path: string): Promise<any> {
  const res = await fetch(`${USA}${path}`, {
    headers: {
      Accept: 'application/json',
      'User-Agent': 'ScriptMasterLabs-gov-procurement-intel/1.1',
...
async function searchAwards(input: {
  ...
  if (input.agency) filters['awarding_agencies'] = [{ name: input.agency, type: 'awarding' }];
  if (input.naics) filters['naics_codes'] = [input.naics];
  ...
  return {
    ...
    total_award_value_usd: Math.round(awards.reduce((s: number, a: any) => s + (a.amount_usd || 0), 0) * 100) / 100,
    filters: input,
    awards,
...
async function matchSetasides(input: { agency?: string; top_n?: number }) {
  const top_n = Math.min(Math.max(input.top_n ?? 15, 1), 50);
  const filters: Record<string, unknown> = {
    award_type_codes: ['A', 'B', 'C', 'D'],
    time_period: [{ start_date: FY_START, end_date: FY_END }],
  ...
  return {
    ...
    awards_returned: awards.length,
    total_setaside_value_usd: Math.round(awards.reduce((s: number, a: any) => s + (a.amount_usd || 0), 0) * 100) / 100,
    awards,
    source: 'USAspending.gov',
    ...sdvosbBlock(),
  };
...
async function agencySpend(input: { limit?: number }) {
...
async function searchSamOpportunities(input: {
  ...
  if (input.keyword) q.set('title', input.keyword);
  if (input.naics) q.set('ncode', input.naics);
  if (input.ncode) q.set('ncode', input.ncode);
  if (input.ptype) q.set('ptype', input.ptype);
  else q.set('ptype', 'o'); // solicitations default
...
async function samEntityLookup(input: { uei?: string; cage?: string; entity_name?: string }) {
  const key = process.env['SAM_API_KEY'] ?? process.env['SAM_KEY'] ?? '';
  if (!key) {
    return {
      error: 'sam_api_key_missing',
  ...
  if (input.uei) q.set('ueiSAM', input.uei);
  if (input.cage) q.set('cageCode', input.cage);
  if (input.entity_name) q.set('legalBusinessName', input.entity_name);
  ...
  const data: any = await res.json().catch(() => ({ error: `HTTP ${res.status}` }));
  if (!res.ok) return { error: `SAM entity HTTP ${res.status}`, body: data, ...sdvosbBlock() };
  return {
    timestamp: new Date().toISOString(),
...
function exportCaptureLog(input: { since?: string }) {
  ensure();
  const raw = readFileSync(LEDGER, 'utf8').trim();
  let entries: LedgerEntry[] = raw ? raw.split('\n').map((l) => JSON.parse(l)) : [];
  if (input.since) {
  ...
  for (const e of entries) {
    const { hmac, ...rest } = e;
    if (sign(rest as Omit<LedgerEntry, 'hmac'>) === hmac) valid++;
    else invalid++;
  }
  ...
  return {
    ...
    procurement_language:
      'Signed agent capture-research log for pilot packages, SDVOSB capability attachments, and internal BD audit.',
  };
}

function gateFree(tool: string, x_payment?: string) {
  if (x_payment) return { allowed: true, tier: 'paid_header' as const };
  const f = freeRemaining(tool);
  if (!f.ok) {
    return {
      allowed: false,
      tier: 'free_exhausted' as const,
  ...
  return { allowed: true, tier: 'free' as const, used: f.used, limit: f.limit };
}
...
async function main() {
  ...
  const run = async (tool: string, params: unknown, x_payment: string | undefined, fn: () => Promise<any>) => {
    const g = gateFree(tool, x_payment);
    if (!g.allowed) return text(g, true);
    try {
      const data = await fn();
      if (g.tier === 'free') consumeFree(tool);
      const digest = createHash('sha256').update(JSON.stringify(params)).digest('hex').slice(0, 12);
      const count = Array.isArray((data as any)?.awards)
        ? (data as any).awards.length
        : Array.isArray((data as any)?.agencies)
          ? (data as any).agencies.length
          : Array.isArray((data as any)?.matches)
            ? (data as any).matches.length
            : undefined;
  ...
  server.tool(
    ...
    'Live SAM.gov solicitations/opportunities (requires SAM_API_KEY). Filter by keyword, NAICS, posted dates.',
    {
  ...
  server.tool(
    'sam_entity_lookup',
    'SAM.gov entity registration lookup by UEI, CAGE, or legal name (requires SAM_API_KEY).',
    {
...
server.tool(
    ...
    'Search federal contract awards (USAspending). Filter by agency, NAICS, keyword, min amount. Free: 3/day.',
    {
  ...
  server.tool(
    'match_setasides',
    'SDVOSB/small-business oriented set-aside award feed for capture research. Free: 3/day.',
    {
      agency: z.string().optional(),
      top_n: z.number().optional(),
      x_payment: z.string().optional(),
  ...
  server.tool(
    'agency_spend_snapshot',
    'Top-tier federal agency obligation snapshot from USAspending. Free: 3/day.',
    {
      limit: z.number().optional(),
      x_payment: z.string().optional(),
    },
  ...
  server.tool(
    'verify_contractor_entity',
    'Lookup federal recipient/contractor entity matches for capture research.',
    {
  ...
  server.tool(
    'export_capture_log',
    'Export HMAC-signed audit trail of this agent\'s procurement research calls (gov/enterprise ready).',
    {
  ...
  server.tool(
    'gov_intel_status',
    'Free status: product one-liner, free quotas, SDVOSB operator ids.',
    {},
    async () => {
