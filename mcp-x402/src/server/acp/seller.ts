/**
 * ScriptMasterLabs ACP Seller — Virtuals Protocol seller agent
 * Service: mcp-x402 | Agent: scriptmasterlabs (0x7233…)
 *
 * Former codename "Leviathan" retired — old agent brand. This module is the
 * live ACP seller for scriptmasterlabs on Render service `mcp-x402`.
 *
 * MARKETPLACE: Virtuals agent NFT / graduation is separate from this process.
 * virtualAgentId 106978 / erc8004 may still need dashboard steps.
 *
 * Buyers pay USDC on Base via Virtuals ACP v2. Calls forward to this server
 * with X-Sml-Acp-Key (or legacy X-Leviathan-Key) bypass for settled jobs,
 * or X-API-Key for SqueezeOS signal routes.
 *
 * Required env vars:
 *   ACP_WALLET_ADDRESS      — scriptmasterlabs wallet (default 0x7233…)
 *   ACP_WALLET_ID           — Privy wallet id (odh4cz…)
 *   ACP_SIGNER_PRIVATE_KEY  — PKCS8 base64 from Virtuals Signers tab
 *   SML_ACP_BYPASS_SECRET | LEVIATHAN_BYPASS_SECRET — x402 bypass (either)
 *   SML_ACP_BASE_URL | LEVIATHAN_BASE_URL — public URL of this server
 *   SQUEEZEOS_API_BASE, SML_API_KEY — optional SqueezeOS backends
 */
...
const SIGNER_PRIVATE_KEY = (process.env['ACP_SIGNER_PRIVATE_KEY'] ?? '')
  ...
  .replace(/^["']|["']$/g, '')
  .replace(/\s+/g, '')
  .replace(/^wallet-auth:/i, '')
  .replace(/^wallet-api:/i, '');
...
async function callMcp(
  ...
  const headers: Record<string, string> = {
    ...
    'X-Agent-DID': 'did:sml:acp:scriptmasterlabs',
  };
...
async function handleEntry(session: JobSession, entry: JobRoomEntry): Promise<void> {
  if (entry.kind === 'system') {
    if (entry.event.type === 'job.funded') {
      ...
      try {
        ...
        await session.reject(`SML-ACP error: ${(err as Error).message}`);
      }
    }
    return;
  }
  ...
  if (entry.kind === 'message' && entry.contentType === 'requirement' && session.status === 'open') {
    ...
    const resolved = resolveOffering(rawDescription);
    if (!resolved) {
      await session.reject(`scriptmasterlabs does not offer: ${rawDescription}`);
      return;
    }
...
export async function startAcpSeller(): Promise<void> {
  ...
  if (!SML_API_KEY) {
    console.warn('[SML-ACP] SML_API_KEY is not set — SqueezeOS signal calls will fail auth');
  }
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('acp.virtuals.io') || url.includes('privy.io')) {
      ...
      clone.text().then(body => {
        console.log(`[SML-ACP] ${init?.method ?? 'GET'} ${url} → ${res.status} ${body.slice(0, 400)}`);
      }).catch(() => {});
    }
    return res;
  };

  console.log(`[SML-ACP] walletId=${WALLET_ID} walletAddress=${WALLET_ADDRESS}`);
  if (WALLET_ADDRESS.toLowerCase() !== '0x72330994f379a71542e7bd5a4cf99a9d9743f4aa'.toLowerCase()) {
    console.warn(`[SML-ACP] WARNING: ACP_WALLET_ADDRESS=${WALLET_ADDRESS} is not scriptmasterlabs primary 0x7233… — sign-typed-data 500s often mean walletId/signer/address mismatch`);
  }
  ...
  try {
    await seller.start(() => {
      console.log('scriptmasterlabs ACP seller online — 54 offerings on Virtuals ACP marketplace');
    ...
    const e = err as Error & { details?: unknown; statusCode?: number; shortMessage?: string };
    console.error('[SML-ACP] seller.start failed:', e.message,
      e.shortMessage ?? '', JSON.stringify(e.details ?? ''));
    throw err;
  }
}
