...
// market data from Polygon.io directly and compute RSI/Delta locally, then
// run a real multi-agent Claude swarm over the result.

import { fetchEquityCloses, fetchOptionsChainSnapshot, type EquityTimeframe } from '../market-data/polygon.js';
...
export const EquitiesHeatmapAPI = {
  ...
  async preview(): Promise<{ tool: string; tier: 'free'; timeframe: EquityTimeframe; heatmap: HeatmapResult }> {
    if (!POLYGON_API_KEY) throw new Error('not_configured: missing POLYGON_API_KEY on this server');
    ...
    return { tool: 'equities_heatmap_preview', tier: 'free', timeframe: '1h', heatmap };
  },
...
export const OptionsDeltaHeatmapAPI = {
  /** Free preview: 5 SPY call contracts, 1 group, no AI swarm. */
  async preview(underlying = 'SPY'): Promise<{ tool: string; tier: 'free'; underlying: string; underlyingPrice: number; heatmap: HeatmapResult }> {
    if (!POLYGON_API_KEY) throw new Error('not_configured: missing POLYGON_API_KEY on this server');
    ...
    if (chain.contracts.length === 0 || chain.underlyingPrice <= 0) {
      throw new Error(`no_data: no options contracts returned for ${underlying}`);
    }
    ...
    return { tool: 'options_delta_heatmap_preview', tier: 'free', underlying: chain.underlying, underlyingPrice: chain.underlyingPrice, heatmap };
  },
