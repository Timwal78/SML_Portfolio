// Groups scored items (RSI, Delta, ...) into the "overbought / oversold"
// heatmap shape: N groups, each with its members and avg/min/max, plus the
// thresholds that define the overbought/oversold bands.
...
export interface BuildHeatmapOptions {
  /** Number of groups to split items into. Defaults to 4 (Group A-D). */
  groupsOf?: number;
  overboughtThreshold?: number;
  oversoldThreshold?: number;
  scale?: string;
